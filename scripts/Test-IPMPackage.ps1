...
if ($manifest.Export.Document.name -ne 'meu-portal.ZPM') {
    throw 'The IPM document name must be meu-portal.ZPM.'
}
if ($module.Name -ne 'meu-portal') {
    throw 'The IPM module name must be meu-portal.'
}
if ($module.SourcesRoot -ne 'src/objectscript') {
    throw 'The IPM SourcesRoot does not point to the ObjectScript source directory.'
}
if ($module.Resource.Name -notcontains 'MeuPortal.PKG') {
    throw 'The MeuPortal.PKG resource is missing from module.xml.'
}
if (-not (Test-Path -LiteralPath (Join-Path $sourceRoot 'MeuPortal\Installer.cls'))) {
    throw 'MeuPortal.Installer is missing.'
}
...
$assetReferences = Select-String -LiteralPath (Join-Path $webRoot 'index.html') -Pattern '/meuportal/assets/[^"'']+' -AllMatches
...
foreach ($match in $assetReferences.Matches) {
    $relativePath = $match.Value.Substring('/meuportal/'.Length).Replace('/', [IO.Path]::DirectorySeparatorChar)
...
Write-Host "ObjectScript classes included by MeuPortal.PKG: $classCount" -ForegroundColor Green
