...
### Step 1 — Install the backend (ObjectScript classes)

1. In that IRIS instance, create (or select) a namespace named `MEUPORTAL`.
2. Import and compile every class under `src/objectscript` into that namespace. The easiest way is the InterSystems ObjectScript extension for VS Code, which imports and compiles automatically when a class is saved while connected to that namespace.
3. From an authenticated IRIS terminal in `MEUPORTAL`, run:
   ...
   This one command creates everything Meu Portal needs: the `/meuportal/api` REST application, the security resources and roles described in [Signing in](#signing-in--which-account-to-use) below, an audit event for administrative changes, six starter health patterns, and a demonstration Task Manager task. It is safe to run again later — for example after an update — since it only creates or updates what is missing.

Alternatively, the whole project can be installed as one IPM package instead of steps 2–3 above — see [Installing through IPM](#installing-through-ipm-alternative-to-step-1) below.

### Step 2 — Install the frontend (the web interface)
...
This script builds the React application and deploys it automatically:

1. installs frontend packages when needed;
2. runs the TypeScript checks and the optimized Vite build;
3. copies the built files to `<IRIS installation>/CSP/meuportal`;
4. adds the Meu Portal rules to the private web server's configuration;
5. validates that configuration;
6. restarts the private IRIS web server so the new files are served.

If IRIS is not installed in the default location, point the script at it:

```powershell
.\scripts\Install-MeuPortal.ps1 -IrisInstallDir 'D:\InterSystems\IRIS'
```
...
### Installing through IPM (alternative to Step 1)
...
- **ObjectScript changes are missing:** confirm that VS Code is connected to `MEUPORTAL` and compile-on-save succeeded.
